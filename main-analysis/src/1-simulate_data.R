...
getargs <- function() {
  ...
  parser$add_argument("--thetagraph2", default="output/theta2.png")
...
gammar <- 0.05 # recovery parameter
N <- 1000000 # pop size
...
theta.df <- data.frame(t=1:length(theta) - 1, theta)

png(args$thetagraph, width=750, height=300)
ggplot(theta.df, aes(x=t, y=Freq)) +
...
## simulating alternative theta with dependence between onset to death and time to onset
scale <- rgamma(100000, incubation - mean(incubation) + 27.75, 1.8)
onset.to.death2 <- rpois(100000, scale)
hist(onset.to.death2)
median(onset.to.death2)
quantile(onset.to.death2, prob=c(0.25, 0.75))

samps2 <- onset.to.death2 + incubation
max.time2 <- quantile(samps2, prob=0.99)-1
theta2 <- table(c(samps2, 0:max.time))[0:max.time2 + 1]
theta2 <- theta2 / sum(theta2)

theta2.df <- data.frame(t=1:length(theta2) - 1, theta2)

png(args$thetagraph2, width=750, height=300)
ggplot(theta2.df, aes(x=t, y=Freq)) +
    geom_bar(stat='identity', width=0.8) +
    theme_bw(base_size=25) +
    xlab("Days from Infection to Death") +
    ylab("Probability")
dev.off()
saveRDS(theta2, file=args$theta2)
...
plot(Ss, type = 'l', col='black', ylim=c(0, 1))
lines(Is, col = 'blue')
lines(Rs, col = 'green')

df.long <- data.frame(S=Ss, I=Is, R=Rs, T=1:T, Nu=Nus, D=Ds[1:T]) %>%
  mutate(I.scale=(I / max(I)) * max(Ds),
         Nu.scale=(Nu / max(Nu)) * max(Ds)) %>%
...
df.long$variable <- factor(df.long$variable,
                           ordered=TRUE,
                           levels=c("D", "Nu", "S", "I", "R", "Nu.scale", "I.scale"))
...
ggsave(args$example_w_nu_pdf, p, width=300, height=200, units="mm")
ggsave(args$example_w_nu_png, p, width=300, height=200, units="mm")
