...
# eliminate max.time, theta
propose.sir.state <- function(R0, gammar, I, T, p, Nus, Ds, xi, zeta, ll.curr, SI,
...
# propose new parameters of SIR model
  ...
  #zeta.T0 : parameters of prior on T0
     ...
     d <- 4
     x <- c(R0, gammar, T0, phi)
  
    if (k < k.adapt) {
       #prop.x <- rnorm(d, x, sqrt(c0))
      prop.x <- rmvnorm(1, x, c0)
    }
    if (k == k.adapt) {
  ...
  if (!is.infinite(lp.prop)) {
    ...
    sim1 <- simulate.sir.timebreak(R0.prop*gammar.prop, gammar.prop, I, N, T0.prop, T, T1, phi.prop)
  ...
  return(list(R0=R0, Nus=Nus, gammar=gammar, acc=acc, X.bar=X.bar, SXXt=SXXt,
              T0=T0, SI=SI, ll.curr=ll.curr, phi=phi))
}



run.mcmc.state <- function(nmc, theta, R0, gammar, T, I, Ds, Nus, xi, zeta, disp.int,
...
# the big function that runs mcmc to estimate beta, gamma, and T0 from data
  ...
  sim1 <- simulate.sir.timebreak(R0*gammar, gammar, I, N, T0, T, T1, phi)
  Nus <- sim1$Nus
  SI <- sim1$SI

  lp.curr <- (log.prior(R0, xi)
              + log.gprior(1 / gammar, zeta)
              + log.T0prior(T0, zeta.T0)
  ...
  for (t in 1:nmc) {
    # sample parameters
    eta <- propose.sir.state(R0, gammar, I, T, p, Nus, Ds, xi, zeta, ll.curr,
