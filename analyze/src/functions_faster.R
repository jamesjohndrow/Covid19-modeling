...
#
# -----------------------------------------------------------
# Covid19-modeling/analyze/src/functions_faster.R
#
...
propose.sir <- function(beta, gammar, I, T, p, Nus, Ds, xi, zeta, ll.curr, SI,
  ...
  return(list(beta=beta, Nus=Nus, gammar=gammar, acc=acc, mn.bg=mn.bg,
              SXXt=SXXt, T0=T0, SI=SI, ll.curr=ll.curr))
...
run.mcmc <- function(nmc, theta, beta, gammar, T, I, Ds, Nus, xi, zeta, disp.int,
  ...
  for (t in 1:nmc) {
    ...
    if (t %% disp.int == 0) {
      ...
      print(paste('acceptance rate in the last',
                  disp.int,
                  'iterations was',
