...
##----- run MCMC
mcmc.out <- run.mcmc(nmc=nmc,theta=theta,beta=beta,gammar=gammar, T=T,I=I,Ds=Ds,Nus=Nus,xi=xi,zeta=zeta,disp.int=disp.int,
...
plot(apply(death.samples, 1, mean), Ds, ylim=c(0, max(Ds)*1.2), xlim=c(0, max(Ds)*1.2))
abline(0,1)
...
sim1 <- simulate.sir(BETA.thinned[1],GAMMAR.thinned[1],SIS.thinned[1,2],N,1,n.days.future+days.since.mitigation+1,SIS.thinned[1,1])
# looks good
plot(c(NUS.thinned[1,1:(T-days.since.mitigation-1)],sim1$Nus))

pacman::p_load(future,future.apply)
plan(multisession)
param.list <- split(cbind(BETA.thinned,GAMMAR.thinned,T0.thinned,SIS.thinned,NUS.thinned),seq(1:n.thin))

for (j in 1:n.cases) {
  print(j)
  phi <- 1-per_decrease[j]
  sir.predict <- future_sapply(param.list,function(x) {c(x[6:(T-days.since.mitigation-1+5)],
                                                         simulate.sir(x[1]*phi,x[2],x[5],N,1,1+days.since.mitigation+n.days.future,x[4])$Nus)})
