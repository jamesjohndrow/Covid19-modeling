...
pacman::p_load(ggplot2,dplyr,tidyr,readr,lubridate, future,future.apply)
#setwd('~/Dropbox/Projects/Covid19-undercount/analyze/')
...
##----- set up
p <- .01 # the IFR. This is an input/assumption of the model.
R0.case <- seq(from=.75,to=2.25,by=0.25) #different R0 cases to consider
...
pops <- readRDS('./input/country_pops.RData')
...
mcmc.out <- run.mcmc(nmc=nmc,theta=theta,beta=beta,gammar=gammar, T=T,I=I,max.time=max.time,Ds=Ds,Nus=sim1$Nus[1:T],xi=xi,zeta=zeta,disp.int=disp.int,
...
NUS <- mcmc.out$NUS
tmp <- simulate.sir(N,mean(mcmc.out$BETA), mean(mcmc.out$GAMMAR),I,T,max.time,theta,p,floor(mean(mcmc.out$T0S)))
plot(tmp$Ds[1:T],Ds)
abline(0,1)
...
tmp1 <- future_lapply(BGt,function(x){simulate.sir(N,x[1],x[2],I,T+n.days.future,max.time,theta,p,floor(x[3]))})
...
tmp2 <- future_lapply(BGt,function(x){simulate.sir(N,x[1],x[2],I,T+n.days.future,max.time,theta,p,ceiling(x[3]))}) #

#extract the samples of number of deaths under this model
tmp01 <- sapply(tmp1,function(x){return(x$Ds[1:(T+n.days.future)])}); tmp02 <- sapply(tmp2,function(x){return(x$Ds[1:(T+n.days.future)])})
#save the samples of D we just made (in lines 124-125)
big.Ds[,,n.cases+1] <- tmp01*(T0.thinned-floor(T0.thinned))+tmp02*(ceiling(T0.thinned)-T0.thinned) #put simulation with actual parameters in last position

#extract samples Nu from each sample run
tmp01 <- sapply(tmp1,function(x){return(x$Nu[1:(T+n.days.future)])}); tmp02 <- sapply(tmp2,function(x){return(x$Nu[1:(T+n.days.future)])});
#save the extracted Nus
big.Nus[,,n.cases+1] <- tmp01*(T0.thinned-floor(T0.thinned))+tmp02*(ceiling(T0.thinned)-T0.thinned) #put simulation with actual parameters in last position
...
for (k in 1:n.cases) {
  print(k)
  tmp1 <- future_lapply(BGt,function(x){simulate.sir(N,c(rep(x[1],T-days.since.mitigation),
  ...
  tmp2 <- future_lapply(BGt,function(x){simulate.sir(N,c(rep(x[1],T-days.since.mitigation),
  ...
  tmp01 <- sapply(tmp1,function(x){return(x$Ds[1:(T+n.days.future)])}); tmp02 <- sapply(tmp2,function(x){return(x$Ds[1:(T+n.days.future)])})
  big.Ds[,,k] <- tmp01*(T0.thinned-floor(T0.thinned))+tmp02*(ceiling(T0.thinned)-T0.thinned)

  #extract Nus
  tmp01 <- sapply(tmp1,function(x){return(x$Nus)}); tmp02 <- sapply(tmp2,function(x){return(x$Nus)});
  big.Nus[,,k] <- tmp01*(T0.thinned-floor(T0.thinned))+tmp02*(ceiling(T0.thinned)-T0.thinned)
}
