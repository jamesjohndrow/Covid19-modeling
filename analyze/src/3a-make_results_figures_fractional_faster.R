...
args[['trace_plots']] <- here::here(glue('analyze/output/trace_plots_fractional_fast_{args$country}_{args$p}.png'))
args[['post_hists']] <- here::here(glue('analyze/output/post_hists_fractional_fast_{args$country}_{args$p}.png'))
...
args[['num_deaths_cases']] <- here::here(glue('analyze/output/num_deaths_cases_fractional_fast_{args$country}_{args$p}.png'))
args[['num_newly_infected']] <- here::here(glue('analyze/output/num_newly_infected_cases_fractional_fast_{args$country}_{args$p}.png'))
args[['predicted_vs_actual']] <- here::here(glue('analyze/output/Predicted_vs_actual_fractional_fast_{args$country}_{args$p}.png'))
args[['R0']] <- here::here(glue('analyze/output/R0_fractional_{args$country}_fast_{args$p}.png'))
args[['infected_10']] <- here::here(glue('analyze/output/infected_10_days_ago_fractional_fast_{args$country}_{args$p}.png'))
args[['newly_infected_10']] <- here::here(glue('analyze/output/newly_infected_10_days_ago_fractional_fast_{args$country}_{args$p}.png'))
args[['time_to_recovery']] <- here::here(glue('analyze/output/time_to_recovery_fractional_fast{args$country}_{args$p}.png'))
...
dt.all <- c(min(dat$date) + seq(from=-14, to=-1, by=1),
            dat$date, 
            max(dat$date) +seq(n.days.future))
...
df.Ds <- df.Ds %>% 
  inner_join(df.qlo, by=c('per_decrease', 'day')) %>%
  inner_join(df.qhi, by=c('per_decrease', 'day'))
...
ggplot(df.Ds %>% filter(day > date("2020-03-28") & day <= date("2020-03-28") + n.days.future),
       aes(x=day, y=log10(value), col=per_decrease)) +
  geom_line() +
  theme_bw() +
  theme(text=element_text(size=24)) +
  geom_ribbon(df.Ds %>% filter(day > date("2020-03-28") & day <= date("2020-03-28") + n.days.future),
              aes(ymin=log10(q025), ymax=log10(q975), fill=per_decrease), alpha=0.2) +
  ylim(c(0, 5)) +
  xlab("date")
...
df.Nus <- df.Nus %>%
  ...
  inner_join(df.qhi, by=c('per_decrease', 'day'))
df.Nus <- df.Nus %>%
  mutate(ribbon=day > date("2020-03-20") | (per_decrease == 0 &  day > date("2020-03-15")))
...
ggplot(df.Nus %>% filter(day > date("2020-03-20") - days.since.mitigation & day <= date("2020-03-29") + n.days.future),
       aes(x=day, y=log10(value + 1), col=per_decrease)) +
  geom_line() +
  theme_bw() +
  theme(text=element_text(size=24)) +
  geom_ribbon(data=df.Nus %>% filter(day > date("2020-03-20") - days.since.mitigation & day <= date("2020-03-29") + n.days.future),
              aes(ymin=log10(q025), ymax=log10(q975), fill=per_decrease), alpha=0.2) +
  xlab("date") +
