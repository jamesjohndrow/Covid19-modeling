...
#
# -----------------------------------------------------------
# Covid19-modeling/analyze/src/functions.R
#
if (!require('pacman')) {install.packages('pacman')}
pacman::p_load(mvtnorm)

#sample Xs as in original manuscript, we actually don't do this anymore,
# but I think we should keep it in case for some reason we decide we want
...
simulate.sir <- function(N, beta, gammar, I, T, max.time, theta, p, T0, Ds=NA) {
  ...
  # Ds : optional vector of death counts. If not included, Ds are simulated
  #      directly from model. If known, incorporated into simulated trajectory.
  ...
  return(list(Nus=Nus, Ds=Ds, Ss=Ss, Is=Is, Rs=Rs,
              save.death.times=save.death.times))
}
...
propose.sir <- function(beta, gammar, I, T, max.time, theta, theta.t, p, Nus, Ds, xi, zeta,
  ...
  if (!is.infinite(lp.prop)) {
    ...
    ll.curr <- (sum(dpois(Ds, apply(Nus * p * theta.t, 2, sum), log=T)) +
                  log.prior(beta / gammar, xi) +
...
run.mcmc <- function(nmc, theta, beta, gammar, T, I, max.time, Ds, Nus, xi, zeta,
                     disp.int, k.adapt=0, c0=1, c1=1, T0=1, zeta.T0) {
  ...
  for (t in 1:nmc) {
    ...
    if (t %% disp.int == 0) {
      ...
      print(paste('acceptance rate in the last', disp.int, 'iterations was',
                  round(mean(ACC[(t - disp.int + 1):t]), 3)))
    }
  }
